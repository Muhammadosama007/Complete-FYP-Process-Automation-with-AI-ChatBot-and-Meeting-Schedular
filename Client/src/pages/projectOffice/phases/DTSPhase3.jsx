...
const DTSPhase3 = () => {
    ...
    useEffect(() => {
        const saved = localStorage.getItem("announcements-dts-phase3");
        if (saved) {
            ...
            const initial = [
                {
                    srNo: 1,
                    subject: "DTS Phase 3 Submission",
                    date: "2025-04-20",
                    description: "Submit DTS Phase 3 documents",
                    attachment: null
                }
            ];
            setAnnouncements(initial);
            localStorage.setItem("announcements-dts-phase3", JSON.stringify(initial));
        }

        const savedSubmissions = localStorage.getItem("dts-phase3-submissions");
        if (savedSubmissions) {
            ...
            const defaultSubmissions = [
                {
                    ...
                    endDate: "2025-04-22",
                    fileName: "dts_phase3_strategy.pdf"
                }
            ];
            setSubmissions(defaultSubmissions);
            localStorage.setItem("dts-phase3-submissions", JSON.stringify(defaultSubmissions));
        }
    }, []);
    ...
    const handleAddAnnouncement = (newAnnouncement) => {
        ...
        localStorage.setItem("announcements-dts-phase3", JSON.stringify(updated));
    };
    ...
    const contentMap = {
        ...
        "Material": (
            <DataTable
                ...
                data={[{
                    srNo: 1,
                    material: "DTS Phase 3 Template",
                    description: "Design Test Strategy Template",
                    download: "dts_phase3_template.docx"
                }].map(m => [
                    m.srNo,
                    m.material,
                    m.description,
